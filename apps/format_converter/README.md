...
- **3D 互动角色** — 全屏可拖拽的 3D 角色模型（猫咪 / Shen），支持多模型切换、点击敲打、拖拽甩飞、弹簧物理回归、自动睡觉、心情系统、程序化叫声，伴有神圣背光和"林参"卫星文字环绕
...
│   │   │   └── InteractivePet.jsx  #     3D 互动角色（模型切换/物理/心情/休眠）
│   │   └── index.css               #    Tailwind + 自定义样式
│   ├── public/
│   │   ├── cat_model.glb           #    猫咪模型 (Draco 18MB)
│   │   ├── shen_model.glb          #    Shen 模型 (Draco 31MB)
...
页面右下角有一只 3D 角色，支持多模型切换，具有以下交互：

- **点击敲打** — 棒子动画 + 疼痛/生气反应
- **拖拽甩飞** — 抓住甩飞，全屏物理弹射后弹簧回归原位
- **自动睡觉** — 30 秒无人互动自动闭眼休眠，点一下叫醒
- **心情系统** — 敲打降心情、空闲自动回中，开心冒 ❤️ 生气冒 💢
- **随机动作** — 空闲时伸懒腰、点头、歪头
- **程序化叫声** — Web Audio 合成喵叫，无需音频文件
- **神圣背光** — 金色光晕呼吸式脉动
- **"林参"卫星** — 发光文字绕角色椭圆轨道旋转

### 模型切换

页面右上角可一键切换角色模型（🐱 小猫 / 🗿 Shen）。模型通过 Draco 压缩，并支持 Service Worker 离线缓存。

### 添加新模型

将 `.glb` 文件放到 `frontend/public/`，在 `InteractivePet.jsx` 的 `MODELS` 数组添加一行即可，模型会自动居中并缩放到合适大小。
