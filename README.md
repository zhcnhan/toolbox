...
| 类别 | 支持格式 | 引擎 |
|------|----------|------|
| 📄 文档 | PDF · DOCX · DOC · TXT · MD · HTML · EPUB · RTF | pdf2docx · PyPDF2 · python-docx · weasyprint · markdown · ebooklib · striprtf |
| 🖼️ 图片 | JPG · PNG · WEBP · BMP · GIF · ICO · TIFF · SVG | Pillow · cairosvg · svglib |
| 🎵 音频 | MP3 · WAV · FLAC · OGG · AAC · M4A · WMA · Opus · AIFF | pydub + ffmpeg |
| 🎬 视频 | MP4 · AVI · MKV · MOV · WEBM · FLV · WMV | ffmpeg (subprocess) |
| 📊 数据 | JSON · YAML · CSV · XML · TOML | PyYAML · xmltodict · tomli · stdlib |

> **171/171 转换路径全部通过自动化测试** ✅

**功能亮点：**
- 🚀 **前后端分离** — React 18 + FastAPI，标准的 RESTful 架构
- 🎨 **极致 UI** — 玻璃拟态设计、Framer Motion 动画、Tailwind CSS 深色主题
- 🐱 **3D 互动小猫** — Rapier 物理引擎驱动的页角小猫，可拖拽、拍打、甩飞
...
# 运行测试: docker compose run --rm format-converter python test_resources/run_test.py
```

> Docker 镜像内置全部依赖：FFmpeg + LibreOffice + GTK3/Cairo + 中文字体，无需额外安装。
...
│       │   │   ├── api/                          #       API 客户端
│       │   │   └── components/                   #       组件库
│       │   │       ├── InteractivePet.jsx      #         3D 互动小猫
│       │   │       ├── Character3D.jsx           #         3D 人偶（备用）
│       │   │       ├── Navbar.jsx                #         导航栏
│       │   │       ├── DropZone.jsx              #         拖放上传
...
| **文档转换** | pdf2docx · python-docx · PyPDF2 · weasyprint · markdown · ebooklib · striprtf | PDF/Word/Markdown/HTML/EPUB/RTF 互转 |
| **音频** | pydub + ffmpeg | 9 种音频格式互转 |
| **视频** | ffmpeg (subprocess) | 7 种视频格式互转 |
| **图片** | Pillow · cairosvg · svglib | 8 种图片格式互转（含 SVG） |
| **数据** | PyYAML · xmltodict · tomli · stdlib | 5 种数据格式互转 |
